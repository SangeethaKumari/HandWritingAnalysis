...
def transcribe_audio(audio_bytes, language="en"):
    ...
    try:
        # If language is "auto", let Whisper detect it automatically
        if language == "auto":
...
def main():
    ...
    selected_language = st.sidebar.selectbox(
        ...
        index=0,
        help="Select the language spoken in the audio, or use Auto Detect"
    )
    
    language_code = language_options[selected_language]
    
    if selected_language != "Auto Detect":
        st.sidebar.info(f"Selected: {selected_language} ({language_code})")
    else:
        st.sidebar.info("Whisper will automatically detect the language")
        ...
        if audio_bytes:
            ...
            if st.button("Transcribe & Analyze", type="primary", key="analyze_voice"):
                with st.spinner("Transcribing audio..."):
                    ...
                    result = transcribe_audio(audio_data, language_code)
